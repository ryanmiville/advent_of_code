...
impl FromStr for Signal {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let (p, o) = s.split_once(" | ").ok_or("bad input")?;
        let patterns = p.split_whitespace().map(String::from).collect();
        let output = o.split_whitespace().map(String::from).collect();
        let signal = Signal { patterns, output };
        Ok(signal)
    }
}
