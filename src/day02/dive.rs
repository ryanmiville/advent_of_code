...
fn unsafe_parse_cmds(str: &str) -> impl Iterator<Item = Command> + '_ {
    str.split("\n")
        .map(|s| match s.split_once(' ').expect("bad input") {
            ("forward", v) => Command::Forward(v.parse().unwrap()),
            ("down", v) => Command::Down(v.parse().unwrap()),
            ("up", v) => Command::Up(v.parse().unwrap()),
            _ => panic!("bad input"),
        })
}
